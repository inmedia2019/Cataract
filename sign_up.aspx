...
<head runat="server">
	...
	<body>
		...
		<script type="text/javascript">
			...
			$(".butsign").click(function() {
				...
				if (tel == "") {
					alert("请输入手机号！")
					//alert_Tips("请输入手机号！", "", 1500)
					return false;
				}
				if (!isPhoneNo(tel)) {
					alert("手机号格式不正确！")
					//alert_Tips("手机号格式不正确！", "", 1500)
					return false;
				}

				if (code == "") {
					alert("请输入验证码！")
					//alert_Tips("请输入验证码！", "", 1500)
					return false;
				}
                ...
                {
                   alert("验证码输入错误！")
                   //alert_Tips("验证码输入错误！", "", 1500)
				   return false;
                }

                $.ajax({
		            type: "POST",
		            url: 'modifyPhoneInfo.aspx',
		            async: false,
		            data: {"tel": tel },
		            beforeSend: function (xmlHttpRequest) {
		            },
		            success: function (data, textStatus) {
		                ...
		                if (data == "1")
		                    window.location.href = "entrance.aspx";
							//teacher_videoList.aspx
		                else
		                	alert("验证码输入错误！")
		                    //alert_Tips("验证码输入错误！", "", 2000)
		            },
		            complete: function (xmlHttpRequest, textStatus) {
		            },
		            error: function () {
   ...
   <script type="text/javascript">
       loadfx('https://wx.linkmed.cn/iCataract_line/phone_login.aspx', '<%=fenxiang.appid%>', '<%=fenxiang.timestamp %>', '<%=fenxiang.nonceStr %>', '<%=fenxiang.signature %>', '<%=linkhead %>', "2022年度强生眼力健白内障手术达人赛")
   </script>
