				...
				<div class="swiper-wrapper">
					<div class="swiper-slide">
						...
						<div class="p1220">
							...
							<div class="h10" <%=counts1>=10?"style='display:none;'":"" %>></div>
							<div class="bfff p1220 bor5500 o85" <%=counts1>=10?"style='display:none;'":"" %>> 作品评分
							</div>
							<div class="h1" <%=counts1>=10?"style='display:none;'":"" %>></div>
							<div class="bfff p1220 bor0055" <%=counts1>=10?"style='display:none;'":"" %>>
								<div class="center">
									<div style="width: calc(100% - 80px);">术前检查&患者筛选(满分20)</div>
								...
								<div class="h10"></div>
								<div class="center">
									<div style="width: calc(100% - 80px);">诊疗思路及手术方式、屈光IOL选择的合理性(满分20)</div>
									<div class="center pr ninput on" style="width: 80px;">
								...
								<div class="h10"></div>
								<div class="center">
									<div style="width: calc(100% - 80px);">病史资料（术前、术中、术后）完整、清晰(满分20)</div>
									<div class="center pr ninput on" style="width: 80px;">
								...
								<div class="h10"></div>
								<div class="center">
									<div style="width: calc(100% - 80px);">手术视频清晰度、剪辑是否得当(满分20)</div>
									<div class="center pr ninput on" style="width: 80px;">
								...
								<div class="center pr ninput on" style="width: 100%;">
									<textarea rows="" id="py" cols="" placeholder="专家评语...(最少15个字)"><%=py %></textarea>
								</div>
							</div>
							<div class="h10" <%=counts1>=10?"style='display:none;'":"" %>></div>
							<div class="center butsign f16" <%=counts1>=10?"style='display:none;'":"" %>>提交</div>
							<div class="h10"></div>
						</div>
					</div>
				</div>
...
iframe.parentNode.removeChild(iframe);
		...
		<script type="text/javascript">
			...
			$(function() {
				...
				$(".butsign").click(function() {
					...
					if(in1 == "") {
						alert("请对术前检查&患者筛选进行打分！")
						return false;
					}

					if(in2 == "") {
						alert("请对诊疗思路及手术方式、屈光IOL选择的合理性进行打分！")
						return false;
					}
					...
					if(in4 == "") {
						alert("请对病史资料（术前、术中、术后）完整、清晰进行打分！")
						return false;
					}
					...
					if(parseInt(in1) > 20) {
						...
						alert("术前检查&患者筛选最多可打20分！")
						return false;
					}

					if(parseInt(in2) > 20) {
						//alert_Tips(, "", 1500)
						alert("诊疗思路及手术方式、屈光IOL选择的合理性最多可打20分！")
						return false;
					}
					...
					if(parseInt(in4) > 20) {
						//alert_Tips(, "", 1500)
						alert("病史资料（术前、术中、术后）完整、清晰最多可打20分！")
						return false;
					}

					if(parseInt(in5) > 20) {
						//alert_Tips(, "", 1500)
						alert("手术视频清晰度、剪辑是否得当最多可打20分！")
						return false;
					}
					...
					$.ajax({
						...
						success: function(data, textStatus) {
							...
							if(temp[0] == "1") {
								...
								//alert_Tips(, "", 2000)
								alert("您评分的视频已经超过10个,无法继续评分！")
							} else if(temp[0] == "6") {
								//alert_Tips(, "", 2000)
								alert("您已完成10个参赛作品的评分，感谢您对达人赛的大力支持！")
								window.location.href = "userScoringlist.aspx";
							}
				...
				$("#overflow_hidden input").bind('input propertychange', function() {
					...
					if(id == "in4" && $(this).val() > 20) {
						//alert_Tips(, "", 1500)
						alert("病史资料（术前、术中、术后）完整、清晰最多可打20分！")
						$(this).val("")
					}
					if(id == "in5" && $(this).val() > 20) {
						//alert_Tips("", "", 1500)
						alert("手术视频清晰度、剪辑是否得当最多可打20分！")
						$(this).val("")
					}
				...
				$("#loadImg img").click(function() {
					...
					for(var i = 0; i < $("#loadImg img").length; i++) {
						imgarr.push($("#loadImg img").eq(i).attr("src"));
					}
					console.log(imgsrc)
					console.log(imgarr)
		...
		<script type="text/javascript">
		    loadfx('http://jjsv.atline.cn/iCataract_line/scoring.aspx?id=<%=id %>', '<%=fenxiang.appid%>', '<%=fenxiang.timestamp %>', '<%=fenxiang.nonceStr %>', '<%=fenxiang.signature %>', '<%=linkhead %>', "2020年度强生眼力健白内障手术达人赛")
		</script>
	</body>
